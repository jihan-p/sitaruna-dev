...
export default function AuthenticatedLayout({ user: authUser, header, children }) {
    ...
    // Efek untuk sinkronisasi expanded state
    useEffect(() => {
        if (!isMobile) {
            if (isSidebarLocked && !isSidebarExpanded) {
                setIsSidebarExpanded(true);
            } else if (!isSidebarLocked && isSidebarExpanded) {
    ...
    // Efek untuk mouse events
    useEffect(() => {
        const sidebarElement = sidebarRef.current;
        if (!sidebarElement) return;

        const handleMouseEnter = () => !isMobile && !isSidebarLocked && setIsSidebarExpanded(true);
        const handleMouseLeave = () => !isMobile && !isSidebarLocked && setTimeout(() => setIsSidebarExpanded(false), 50);
    ...
    const toggleSidebarLock = () => {
        ...
        try { localStorage.setItem('sidebarLocked', nextLockedState.toString()); } 
    ...
    const mainContentClasses = `flex-1 flex flex-col overflow-y-auto ${isMobile ? 'ml-0' : 'sm:ml-0'}`;
        ...
        <div className="flex min-h-screen bg-gray-100">
            ...
            <div ref={sidebarRef} className={sidebarClasses}>
                <div className="flex shrink-0 items-center px-4 py-6 border-b border-gray-200 justify-between">
                    ...
                    {!isMobile && (
                        <div className={`flex items-center ml-auto ${!isSidebarExpanded ? 'sm:-mr-4' : ''}`}>
                            ...
                            >
                                {isSidebarLocked ? <IconLock size={20} /> : <IconLockOpen size={20} />}
                            </label>
                        </div>
                    )}
                ...
                <nav className="flex flex-col flex-1 px-2 py-4 space-y-1">
                    ...
                    {/* Daftar NavLink dengan Permission Check */}
                    {auth.user && hasAnyPermission(['roles index']) && (
                        <NavLink
                    ...
                    {auth.user && hasAnyPermission(['permissions index']) && (
                        <NavLink
                    ...
                    {auth.user && hasAnyPermission(['users index']) && (
                        <NavLink
                    ...
                    {auth.user && hasAnyPermission(['students index']) && (
                        <NavLink
                    ...
                    {auth.user && hasAnyPermission(['majors index']) && (
                        <NavLink
            ...
            {isMobile && isMobileSidebarOpen && (
                <div className="fixed inset-0 z-40 bg-black bg-opacity-25" onClick={toggleMobileSidebar}></div>
            )}
                ...
                <footer className="bg-white py-4 text-center mt-8">
                    <div className="max-w-7xl mx-auto sm:px-6 lg:px-8">
                        <p className="text-gray-600 text-sm">
                            ...
                            <IconHeart size={16} className="inline-block align-text-bottom" /> by{' '}
                            <a href="https://t.me/jhanplv" target="_blank" className="text-blue-600 hover:underline">
                                jipi
                            </a>{' '}
                            @RPL SMKN 2 Subang
                        </p>
